...
# todo: implement
def add_lang_props_to_resource(key, value, filename):
    print("add_lang_props_to_resource: not implemented!")

es_dict = build_es_dict(es)
web_path = 'c:/Users/jiang/git/pekall/web/mdm/web-admin'


soup = BeautifulSoup(open(web_path + '/device/device_list.html', 
...
for tag in soup.find_all(tagWithChinseAttrs):
    for k, v in tag.attrs.items():
        if isinstance(v, str) and zh_regex.search(v):
            ...
            add_lang_props_to_resource(trans_key, trans_value, )
            # todo: 
            break
